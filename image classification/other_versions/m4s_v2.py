...
def build_CNN_classifier(x):
    ...
    l2_flat = tf.reshape(l2_pool, [-1, 8 * 8 * 128])

    W_fc1 = tf.get_variable(name="W_fc1", shape=[8 * 8 * 128, 10], initializer=tf.contrib.layers.xavier_initializer())
    b_fc1 = tf.get_variable(name="b_fc1", shape=[10], initializer=tf.contrib.layers.xavier_initializer())
    logits = tf.nn.bias_add(tf.matmul(l2_flat, W_fc1), b_fc1)
