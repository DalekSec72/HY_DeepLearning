...
def build_CNN_classifier(x):
    ...
    W_fc1 = tf.get_variable(name="W_fc1", shape=[4 * 4 * 128, 256], initializer=tf.contrib.layers.xavier_initializer())
    b_fc1 = tf.get_variable(name="b_fc1", shape=[256], initializer=tf.contrib.layers.xavier_initializer())
    fcl1 = tf.nn.bias_add(tf.matmul(l3_flat, W_fc1), b_fc1)

    W_fc2 = tf.get_variable(name="W_fc2", shape=[256, 10], initializer=tf.contrib.layers.xavier_initializer())
    b_fc2 = tf.get_variable(name="b_fc2", shape=[10], initializer=tf.contrib.layers.xavier_initializer())
    logits = tf.nn.bias_add(tf.matmul(fcl1, W_fc2), b_fc2)
